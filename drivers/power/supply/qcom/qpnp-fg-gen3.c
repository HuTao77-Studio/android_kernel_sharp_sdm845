...
#ifdef CONFIG_BATTERY_SHARP
	chip->cl.stored_learned_capacity = false;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	chip->cl.stored_learned_capacity = true;
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	if(!chip->profile_loaded && !chip->cl.learned_cc_uah) {
		int act_cap_mah;
		rc = fg_get_sram_prop(chip, FG_SRAM_ACT_BATT_CAP, &act_cap_mah);
		if (rc < 0) {
			pr_err("Error in getting ACT_BATT_CAP, rc=%d\n", rc);
		...
		chip->cl.learned_cc_uah = act_cap_mah * 1000;
		fg_dbg(chip, FG_CAP_LEARN, "Get learned capacity from sram %lld\n", chip->cl.learned_cc_uah);
	}
#endif /* CONFIG_BATTERY_SHARP */
