...
static void shtps_func_setlpwg_doubletap(struct shtps_rmi_spi *ts, u8 on)
{
...
static void shtps_func_doubletap_on(struct shtps_rmi_spi *ts)
{
	shtps_func_setlpwg_doubletap(ts, 1);
}

static void shtps_func_doubletap_off(struct shtps_rmi_spi *ts)
{
	shtps_func_setlpwg_doubletap(ts, 0);
}
...
static void shtps_func_workq( struct work_struct *work_p )
{
	...
	while( list_empty( &(ts->queue) ) == 0 ){
		...
		switch(ts->cur_msg_p->event){
			...
			#if defined(SHTPS_LPWG_DOUBLE_TAP_ENABLE)
				case SHTPS_FUNC_REQ_EVENT_DOUBLETAP_ON:
					shtps_func_doubletap_on(ts);
					ts->cur_msg_p->status = 0;
					break;

				case SHTPS_FUNC_REQ_EVENT_DOUBLETAP_OFF:
					shtps_func_doubletap_off(ts);
					ts->cur_msg_p->status = 0;
					break;
			#endif /* SHTPS_LPWG_DOUBLE_TAP_ENABLE */
...
static char* shtps_func_get_event_name(int event)
{
	switch(event){
		...
		case SHTPS_FUNC_REQ_EVENT_DOUBLETAP_ON:		return "DOUBLETAP_ON";
		case SHTPS_FUNC_REQ_EVENT_DOUBLETAP_OFF:	return "DOUBLETAP_OFF";
	}
	return "UnKnown";
}
