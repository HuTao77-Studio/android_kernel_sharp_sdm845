...
static int shtps_fwctl_s3700_get_gesturetype(struct shtps_fwctl_info *fc_p, u8 *status_p, u8 *property_p);
...
static int shtps_fwctl_s3700_get_gesturetype(struct shtps_fwctl_info *fc_p, u8 *status_p, u8 *property_p)
{
	...
	SHTPS_LOG_FWCTL_FUNC_CALL();
	rc = M_READ_FUNC(fc_p, fc_p->map_p->fn12.data.num[4].addr, buf, 5);
	if(rc == 0){
		*status_p = buf[0];
		if(property_p != NULL){
			memcpy(property_p, &buf[1], 4);
		}
	}
	else{
		*status_p = SHTPS_GESTURE_TYPE_NONE;
	}
