...
#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
	static DEFINE_MUTEX(shtps_lpwg_notify_wake_lock_mutex);
#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
...
static unsigned long shtps_check_startuptime(struct shtps_rmi_spi *ts);
	...
	#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
		static void shtps_lpwg_notify_wake_lock_timer(struct work_struct *work);
	#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
...
void shtps_mutex_lock_lpwg_notify_wake_lock(void)
{
#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
	TPS_MUTEX_LOG_LOCK("shtps_lpwg_notify_wake_lock_mutex");
	mutex_lock(&shtps_lpwg_notify_wake_lock_mutex);
#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
}

void shtps_mutex_unlock_lpwg_notify_wake_lock(void)
{
#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
	TPS_MUTEX_LOG_UNLOCK("shtps_lpwg_notify_wake_lock_mutex");
	mutex_unlock(&shtps_lpwg_notify_wake_lock_mutex);
#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
}
...
void shtps_ioctl_setlpwg_doubletap_proc(struct shtps_rmi_spi *ts, u8 on)
{
	...
	if(on){
		shtps_func_request_sync(ts, SHTPS_FUNC_REQ_EVENT_DOUBLETAP_ON);
	}else{
		shtps_func_request_sync(ts, SHTPS_FUNC_REQ_EVENT_DOUBLETAP_OFF);
	}
}
...
static void shtps_lpwg_wakelock_init(struct shtps_rmi_spi *ts)
{
	...
	#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
		ts->lpwg.notify_wake_lock_state = false;
		wake_lock_init(&ts->lpwg.notify_wake_lock, WAKE_LOCK_SUSPEND, "shtps_lpwg_notify_wake_lock");
		INIT_DELAYED_WORK(&ts->lpwg.notify_wake_lock_delayed_work, shtps_lpwg_notify_wake_lock_timer);
	#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
}
...
static void shtps_lpwg_wakelock_destroy(struct shtps_rmi_spi *ts)
{
	...
	#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
		cancel_delayed_work(&ts->lpwg.notify_wake_lock_delayed_work);
		wake_lock_destroy(&ts->lpwg.notify_wake_lock);
	#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
}
...
#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
void shtps_lpwg_notify_wake_lock(struct shtps_rmi_spi *ts, int on)
{
	SHTPS_LOG_FUNC_CALL();

	shtps_mutex_lock_lpwg_notify_wake_lock();

	if(on){
		if (ts->lpwg.notify_wake_lock_state == false) {
			ts->lpwg.notify_wake_lock_state = true;
			wake_lock(&ts->lpwg.notify_wake_lock);
			SHTPS_LOG_DBG_PRINT("lpwg notify wake lock ON\n");
		}
	}else{
		if (ts->lpwg.notify_wake_lock_state == true) {
			ts->lpwg.notify_wake_lock_state = false;
			wake_unlock(&ts->lpwg.notify_wake_lock);
			SHTPS_LOG_DBG_PRINT("lpwg notify wake lock OFF\n");
		}
	}

	shtps_mutex_unlock_lpwg_notify_wake_lock();
}

static void shtps_lpwg_notify_wake_lock_timer(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct shtps_lpwg_ctrl *lpwg_p = container_of(dw, struct shtps_lpwg_ctrl, notify_wake_lock_delayed_work);
	struct shtps_rmi_spi *ts = container_of(lpwg_p, struct shtps_rmi_spi, lpwg);
	...
	shtps_lpwg_notify_wake_lock(ts, 0);
}
#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
...
static void shtps_notify_double_tap_event(struct shtps_rmi_spi *ts)
{
...
#if defined(SHTPS_SUPPORT_GESTURE_NOTIFY)
	sysfs_notify(ts->lpwg.shtpsif_kobj, NULL, ts->lpwg.double_tap_notify_attr_name);
#else
	...
	input_sync(ts->input_key);
#endif /* SHTPS_SUPPORT_GESTURE_NOTIFY */

#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
	shtps_lpwg_notify_wake_lock(ts, 1);
	cancel_delayed_work(&ts->lpwg.notify_wake_lock_delayed_work);
	schedule_delayed_work(&ts->lpwg.notify_wake_lock_delayed_work,
						msecs_to_jiffies(SHTPS_LPWG_NOTIFY_WAKE_LOCK_TIMEOUT_MS));
#endif /* SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE */
}
#endif /* SHTPS_LPWG_MODE_ENABLE */
...
static void shtps_read_touchevent_insleep(struct shtps_rmi_spi *ts, int state)
{
	...
	#if defined( SHTPS_HOST_LPWG_MODE_ENABLE )
	{
		...
		if(shtps_check_host_lpwg_enable() == 1){
			...
			shtps_fwctl_get_gesturetype(ts, &val, &property[0]);
		}
	}
	#else
		shtps_fwctl_get_gesturetype(ts, &val, &property[0]);
	...
	if(val == SHTPS_GESTURE_TYPE_ONE_FINGER_SWIPE){
		...
		if(ts->lpwg.notify_enable != 0)
		{
			ts->lpwg.double_tap_x = (property[1] << 8) | property[0];
			ts->lpwg.double_tap_y = (property[3] << 8) | property[2];

			#if defined(SHTPS_COORDINATES_POINT_SYMMETRY_ENABLE)
				ts->lpwg.double_tap_x = shtps_fwctl_get_maxXPosition(ts) - ts->lpwg.double_tap_x;
				ts->lpwg.double_tap_y = shtps_fwctl_get_maxYPosition(ts) - ts->lpwg.double_tap_y;
			#endif /* SHTPS_COORDINATES_POINT_SYMMETRY_ENABLE */
