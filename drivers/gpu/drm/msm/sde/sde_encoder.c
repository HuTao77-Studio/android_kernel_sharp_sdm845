		...
		struct sde_encoder_phys *phy_enc)
{
	...
	switch(fps_low_base) {
		case DRM_BASE_FPS_30:
			phy_enc->fpslow_count += 1;
			break;
		case DRM_BASE_FPS_60:
			phy_enc->fpslow_count += 2;
			break;
		case DRM_BASE_FPS_100:
		case DRM_BASE_FPS_120:
			phy_enc->fpslow_count += 4;
			break;
		default:
	...
	pr_debug("%s: fps_base = %d, fps_count = %d\n",
			__func__, fps_low_base, phy_enc->fpslow_count);

	if (phy_enc->fpslow_count < max_cnt) {
