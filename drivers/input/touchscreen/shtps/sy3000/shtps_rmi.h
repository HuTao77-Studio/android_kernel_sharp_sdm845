...
struct shtps_lpwg_ctrl{		//**********	SHTPS_LPWG_MODE_ENABLE
	...
	struct kobject				*shtpsif_kobj;
	const u8					*double_tap_notify_attr_name;
	unsigned short				double_tap_x;
	unsigned short				double_tap_y;

	#if defined(SHTPS_LPWG_NOTIFY_WAKE_LOCK_ENABLE)
		struct wake_lock			notify_wake_lock;
		struct delayed_work			notify_wake_lock_delayed_work;
		bool						notify_wake_lock_state;
...
enum{
	...
	SHTPS_FUNC_REQ_EVENT_DOUBLETAP_ON,
	SHTPS_FUNC_REQ_EVENT_DOUBLETAP_OFF,
};
