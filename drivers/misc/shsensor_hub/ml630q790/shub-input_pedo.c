...
static bool poll_timer_suspend = false; /* SHMDS_HUB_0367_01 add */
    ...
    switch (cmd) {
        case SHUBIO_PEDO_ACTIVATE:
            ...
            if((batch_param.m_Latency != 0) && (currentActive == 0)){
                ...
                //set polling start 
                poll_timer_suspend = false; /* SHMDS_HUB_0367_01 add */
...
/* SHMDS_HUB_0367_01 add S */
void shub_suspend_pedo(void)
{
    if(hrtimer_active(&poll_timer)){
        printk("%s: timer stop\n", __func__);
        poll_timer_suspend = true;
        shub_pedo_timer_stop();
    }
}

void shub_resume_pedo(void)
{
    if(poll_timer_suspend) {
        printk("%s: timer restert\n", __func__);
        shub_set_sensor_poll(1);
    }
    poll_timer_suspend = false;
}
/* SHMDS_HUB_0367_01 add E*/
...
int shub_pedo_init(void)          /* SHMDS_HUB_3701_01 mod */
{
    int ret;
...
void shub_pedo_exit(void)         /* SHMDS_HUB_3701_01 mod */
{
    platform_driver_unregister(&shub_pedo_driver);
}

// late_initcall(shub_pedo_init); /* SHMDS_HUB_3701_01 del */
// module_exit(shub_pedo_exit);   /* SHMDS_HUB_3701_01 del */
