...
static ssize_t shtpsif_show_wake_event(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct shtps_rmi_spi *ts = gShtps_rmi_spi;

	SHTPSIF_LOG_FUNC_CALL();

	return scnprintf(buf, PAGE_SIZE, "%d,%d\n", ts->lpwg.double_tap_x, ts->lpwg.double_tap_y);
}
SHTPSIF_DEFINE(wake_event, shtpsif_show_wake_event, SHTPSIF_STORE_COMMON);
...
#endif /* SHTPS_ENGINEER_BUILD_ENABLE */
			#if defined(SHTPS_SUPPORT_GESTURE_NOTIFY)
				ts->lpwg.shtpsif_kobj = gShtps_kobj_group_shtpsif_p;
				ts->lpwg.double_tap_notify_attr_name = shtpsif_wake_event.attr.name;
			#endif /* SHTPS_SUPPORT_GESTURE_NOTIFY */
		}
	}
}
